...
redistribute it and/or modify it under the terms of version 3 of the GNU General Public License as published by the Free Software Foundation.
...
(function(window, pbs, $, undefined) {
  ...
  function input(n){
    ...
    if(!String(n).match(/^[1-3]$/)){
      throw new Error('invalid arguments - pbs.input() required one argument - 1, 2, or 3');
    }

    var rawVal = opener.$('#pbs_input_' + n).val();
    if(rawVal.length){
      return rawVal;
    }
    return undefined;
  ...
  // Returns    : an array of strings, which could be of zero length
  // Arguments  : NONE
  // Throws     : NOTHING
  // Notes      :
  // See Also   :
  ...
  function isCodeMirrorObj(o){
    return o instanceof opener.CodeMirror ? true : false;
  }
...
$(document).ready(function(){
  // init the PBS object
  pbs.init(opener.codeEditor, $('#pbs_output'));
...
$(window).unload(function(){
  // blank our variable in our opener
  if(opener){
    opener.PLAYGROUND = null;
  }
});
